...
def main():
    ...
    def finAlert(path, stock_no, strtodate, stryesdate):
        Finmessage2 = tuple()
        try:
            findata = pd.read_csv(path + cf.FinancingData +
    ...
    def periodpeak(path, filename):
        ...
        periodhigh = pd.read_csv(
            path+cf.periodHighDir + filename+"periodhigh.csv", thousands=",")  # thounsands可以去千位符號
        periodlow = pd.read_csv(
            path+cf.periodLowDir+filename+"periodlow.csv")
        # 只查看最近N天內的指標
        filtday = 60
    ...
    for i in range(0, len(fostocklist)):  # len(fostocklist)
        ...
        fulldir = path+cf.Pricedata+hisfilename
